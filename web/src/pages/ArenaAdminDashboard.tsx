    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-8 pt-25">
      <header className="max-w-7xl mx-auto mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
          ...
          <h1 className="text-5xl font-black italic tracking-tighter uppercase mb-2">Arena Command</h1>
        ...
        <div className="flex flex-wrap gap-4">
           ...
           >
              ...
              <div className="text-left">
                 ...
                 <p className="text-xs font-black uppercase italic tracking-tight text-slate-900">Architect</p>
              </div>
           </button>
           ...
           >
              ...
              <div className="text-left">
                 ...
                 <p className="text-xs font-black uppercase italic tracking-tight text-slate-900">Deploy</p>
              </div>
           </button>
           ...
           >
              ...
              <div className="text-left">
                 ...
                 <p className="text-xs font-black uppercase italic tracking-tight text-white">Schedule</p>
              </div>
           </button>
        </div>
      </header>
            ...
            <div className="flex border-b border-slate-100 bg-slate-50/30">
               {(['contests', 'problems', 'bugs', 'stats'] as const).map(tab => (
                 <button 
                  ...
                  className={`px-10 py-8 text-[11px] font-black uppercase tracking-widest transition-all relative ${
                            ...
                            <div key={prob._id} className="bg-slate-50 border border-slate-100 p-8 rounded-[32px] hover:border-pink-600/30 transition-all group shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                               <div className="flex items-center justify-between mb-6">
                                  <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
                                    prob.difficulty === 'Easy' ? 'bg-emerald-100 text-emerald-700' : 
                               ...
                               <h3 className="text-2xl font-black italic tracking-tighter uppercase mb-2 group-hover:text-pink-600 transition-colors leading-tight">{prob.title}</h3>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          {bugs.map((bug) => (
                            <div key={bug._id} className="bg-slate-50 border border-slate-100 p-8 rounded-[32px] hover:border-rose-600/30 transition-all group shadow-sm hover:shadow-xl hover:shadow-slate-200/50">
                               <div className="flex items-center justify-between mb-6">
                                  <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
                                    bug.difficulty === 'Easy' ? 'bg-emerald-100 text-emerald-700' : 
                               ...
                               <h3 className="text-2xl font-black italic tracking-tighter uppercase mb-2 group-hover:text-rose-600 transition-colors leading-tight">{bug.title}</h3>
