...
const ContestDetails: React.FC = () => {
  ...
  if (!contest) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8">
       ...
       <h1 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900">Mission Signal Lost</h1>
                ...
                <header className="mb-12">
                   <div className="flex items-center gap-4 mb-6">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                         isOngoing ? 'bg-pink-500/10 text-pink-600 animate-pulse' : 
                   ...
                   <h1 className="text-6xl font-black italic tracking-tighter uppercase mb-6 leading-tight select-none">
                ...
                <section>
                   ...
                   <div className="space-y-4">
                      {contest.problems?.map((prob: any, i: number) => (
                        <div 
                         key={prob._id}
                         className={`bg-white border border-slate-200 rounded-3xl p-8 flex items-center justify-between group transition-all shadow-sm ${
                            (hasEntered && isOngoing) || isCompleted ? 'cursor-pointer hover:border-pink-500/50 hover:shadow-xl hover:shadow-slate-200/50' : 'opacity-70 grayscale-0 pointer-events-none'
                         }`}
                         onClick={() => {
                        ...
                        >
                           <div className="flex items-center gap-8">
                              <div className={`w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center font-black italic transition-all border border-slate-100 ${
                                 prob.isLocked ? 'text-slate-200' : 'text-slate-300 group-hover:text-pink-600 group-hover:bg-pink-50'
                              }`}>
                                 {prob.isLocked ? <Shield size={18} /> : i + 1}
                              </div>
                              <div>
                                 <h4 className={`text-xl font-black italic tracking-tighter uppercase transition-colors ${
                                    prob.isLocked ? 'text-slate-300' : 'group-hover:text-pink-600'
                                 }`}>{prob.title}</h4>
                ...
                <section className="bg-white border border-slate-200 rounded-[40px] p-10 shadow-xl shadow-slate-200/50">
                   ...
                   <div className="space-y-6 mb-12">
                      <div className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                         ...
                         <span className="text-sm font-black italic">{contest.participants?.length || 0}</span>
                      </div>
                      <div className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                         <div className="flex items-center gap-3">
                         ...
                         <span className="text-sm font-black italic text-pink-600">{contest.prizePool || 'Glory'}</span>
                      </div>
                   </div>
                ...
                <section className="bg-white border border-slate-200 rounded-[40px] p-10 shadow-xl shadow-slate-200/50 overflow-hidden relative">
                   ...
                   <div className="space-y-6">
                      {contest.participants?.slice(0, 5).map((p: any, i: number) => (
                        <div key={i} className="flex items-center justify-between group">
                           <div className="flex items-center gap-4">
                              <span className="text-[10px] font-black italic text-slate-200 group-hover:text-slate-400 transition-colors">#{i + 1}</span>
                      ...
                      {(!contest.participants || contest.participants.length === 0) && (
                        <p className="text-[10px] font-bold text-slate-200 text-center py-6 italic uppercase tracking-widest">Awaiting deployment signals...</p>
                      )}
                   </div>
                </section>
             </div>
