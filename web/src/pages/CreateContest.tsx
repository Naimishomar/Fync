    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-8 pt-25 pb-20">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-12">
          <h1 className="text-5xl font-black  tracking-tighter uppercase mb-4 leading-none">Initialize Mission</h1>
        ...
        <form onSubmit={handleSave} className="space-y-12">
          ...
          <section className="bg-white border border-slate-200 rounded-[48px] p-12 shadow-xl shadow-slate-200/50">
            <div className="flex items-center gap-5 mb-12">
              ...
              <div>
                <h2 className="text-2xl font-black  tracking-tighter uppercase">Mission Briefing</h2>
          ...
          <section className="bg-white border border-slate-200 rounded-[48px] p-12 shadow-xl shadow-slate-200/50">
            <div className="flex items-center justify-between mb-12">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h2 className="text-2xl font-black  tracking-tighter uppercase">Challenge Selection ({selectedProblems.length})</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
              {problems.map((prob) => (
                <div
                  ...
                  className={`p-8 rounded-[40px] border-2 transition-all cursor-pointer flex items-center justify-between group shadow-sm ${selectedProblems.includes(prob._id)
                      ? 'bg-indigo-50 border-indigo-500 shadow-indigo-600/10'
                      : 'bg-white border-slate-100 hover:border-slate-300'
                    }`}
                >
                  <div className="flex items-center gap-6">
                    <div className={`w-12 h-12 rounded-[20px] flex items-center justify-center transition-all ${selectedProblems.includes(prob._id) ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-slate-50 text-slate-300 group-hover:text-slate-500 border border-slate-100'
                      }`}>
                      {selectedProblems.includes(prob._id) ? <Check size={20} strokeWidth={3} /> : <Plus size={20} />}
                    </div>
                    <div>
                      <h4 className="text-lg font-black  tracking-tighter uppercase leading-tight">{prob.title}</h4>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-base font-black  tracking-tighter text-indigo-600 leading-none">{prob.points} PT</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {success && (
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] font-black uppercase  text-slate-900">Contest scheduled successfully</p>
            </div>
            <motion.div
              initial={{ width: '0%' }}
              animate={{ width: '100%' }}
