...
const RecruiterPortal = () => {
    ...
    const { user } = useAuth();
    const [activeTab, setActiveTab] = useState<'posts' | 'applicants'>('posts');
    ...
    const renderPostItem = ({ item }: { item: any }) => (
        <View className="bg-white rounded-3xl m-4 p-5 shadow-sm border border-slate-100">
            ...
            <View className="flex-row items-center justify-between mt-2 pt-4 border-t border-slate-50">
                ...
                >
                    <Text className="text-white font-black italic uppercase text-[10px] tracking-widest">View Candidates</Text>
                </TouchableOpacity>
            </View>
        </View>
    );
    ...
    const renderApplicantItem = ({ item }: { item: any }) => (
        <View className="bg-white rounded-3xl m-4 p-5 shadow-sm border border-slate-100">
            <View className="flex-row items-center gap-4 mb-4">
                <Image source={{ uri: item.candidate?.avatar || 'https://via.placeholder.com/100' }} className="w-14 h-14 rounded-2xl" />
                <View className="flex-1">
            ...
            <View className="flex-row gap-2">
                ...
                <TouchableOpacity 
                    onPress={() => item.portfolioUrl && Linking.openURL(item.portfolioUrl)}
    ...
    return (
        <SafeAreaView className="flex-1 bg-[#F8FAFC]">
            ...
            {selectedPostId && activeTab === 'applicants' && (
                <View className="px-6 flex-row items-center justify-between mb-2">
                    <Text className="text-slate-400 text-[10px] font-bold italic uppercase">Filtering for specific post</Text>
                    <TouchableOpacity onPress={() => setSelectedPostId(null)}>
                        <Text className="text-pink-500 text-[10px] font-black uppercase underline">Clear</Text>
                    </TouchableOpacity>
                </View>
            )}
            ...
            {loading ? (
                ...
                <FlatList
                    data={activeTab === 'posts' ? posts : filteredApplications}
