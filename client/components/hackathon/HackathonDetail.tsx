  ...
  return (
    ...
    >
      <View className="flex-row justify-between items-center">
        <Text className="text-zinc-900 font-black  uppercase text-xs tracking-tight flex-1">{faq.question}</Text>
      ...
      >
        <SafeAreaView>
          <View className="flex-row items-center px-6 py-3">
            <Text className="text-zinc-900 font-black  text-lg uppercase tracking-tight flex-1" numberOfLines={1}>{hackathon.title}</Text>
          </View>
        </SafeAreaView>
      </Animated.View>
      ...
      <SafeAreaView className="absolute top-0 left-0 right-0 z-50 pointer-events-none" pointerEvents="box-none">
        <View className="flex-row items-center justify-between px-6 pt-3">
          ...
          <View className="flex-row gap-3">
            ...
            >
              <Text className="text-white font-black  text-[10px] uppercase tracking-widest">🏆 Leaderboard</Text>
            </TouchableOpacity>
          </View>
        </View>
      </SafeAreaView>
          ...
          <View className="absolute bottom-16 px-8">
            <View className="flex-row items-center mb-3">
              <LinearGradient colors={statusMeta.colors as any} className="px-4 py-1.5 rounded-xl border border-white/20">
                <Text className="text-white font-black  text-[10px] uppercase tracking-widest">{statusMeta.label}</Text>
              </LinearGradient>
            </View>
        ...
        <View className="bg-white -mt-10 mx-6 rounded-[32px] p-6 shadow-2xl shadow-black/10 border border-gray-50 flex-row items-center justify-between">
          <View className="flex-row items-center">
            ...
            <View className="ml-4">
              ...
              <Text className="text-zinc-900 text-sm font-black  uppercase">{hackathon.organiser?.name}</Text>
            </View>
          </View>
        ...
        <View className="mt-4 px-6">
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 15 }}>
            {TABS.map((tab, i) => (
              ...
              >
                <Text className={`font-black  text-[10px] uppercase tracking-widest ${activeTab === i ? 'text-white' : 'text-slate-400'}`}>{tab}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View className="mx-6 mb-40">
          ...
          {activeTab === 0 && (
            <View>
              ...
              <View className="bg-white rounded-[32px] p-8 mb-6 border border-slate-50 shadow-sm">
                <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-4">📖 Mission Protocol</Text>
                ...
                {hackathon.rules && hackathon.rules.length > 0 && (
                  <View>
                    <Text className="text-zinc-900 font-black  text-base uppercase tracking-widest mb-4">Commandments</Text>
              ...
              {hackathon.sponsors?.length > 0 && (
                <View className="bg-white rounded-[40px] p-8 mb-6 border border-slate-50 shadow-sm">
                  <Text className="text-zinc-900 font-black  text-base uppercase tracking-widest mb-6">Strategic Partners</Text>
              ...
              <View className="bg-slate-50 rounded-[40px] p-8 border border-slate-100 shadow-inner">
                <Text className="text-zinc-900 font-black  text-base uppercase tracking-widest mb-6">Official Signals</Text>
          ...
          {activeTab === 1 && (
            <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
              <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-8">🕒 Operation Timeline</Text>
                ...
                hackathon.timeline.map((event: any, i: number) => (
                  <View key={i} className="flex-row gap-6 mb-8 overflow-visible">
                    ...
                    <View className="flex-1 pt-1 pb-4">
                      ...
                      <Text className="text-zinc-900 font-black  text-base uppercase tracking-tight mt-1">{event.label}</Text>
          ...
          {activeTab === 2 && (
            <View>
              <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-6 mx-2">📡 Strategic Tracks</Text>
                ...
                hackathon.tracks.map((track: any, i: number) => (
                  ...
                  >
                    <View className="flex-row items-center justify-between mb-4">
                      <View className="bg-zinc-900 px-4 py-2 rounded-xl"><Text className="text-white font-black  uppercase text-[10px]">Track {i + 1}</Text></View>
                      <View className="flex-row items-center bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100">
                        ...
                        <Text className="text-emerald-600 font-black  uppercase text-[10px] ml-2">{track.prizes || 'Standard Bounty'}</Text>
                      </View>
                    </View>
                    <Text className="text-zinc-900 font-black  text-2xl uppercase tracking-tighter mb-3">{track.title}</Text>
          ...
          {activeTab === 3 && (
            <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
              <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-8">💡 Mission Advisors</Text>
                ...
                <View className="flex-row flex-wrap gap-6">
                  {hackathon.mentors.map((mentor: any, i: number) => (
                    <View key={i} className="items-center w-[28%]">
                      ...
                      <Text className="text-zinc-900 font-black  text-center uppercase text-[10px] tracking-tight">{mentor.name}</Text>
          ...
          {activeTab === 4 && (
            <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
              <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-8">💬 Protocol FAQ</Text>
            ...
            <View>
              {isOfficial() && (
                <View className="bg-white rounded-[40px] p-8 mb-6 border border-pink-100 shadow-xl shadow-pink-500/5">
                  <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter mb-6">📢 Broadcast Signal</Text>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-6">
                    {ANN_TYPES.map(t => (
                      ...
                      >
                        ...
                        <Text className={`font-black  uppercase text-[10px] tracking-widest ml-2 ${annForm.type === t.value ? 'text-white' : 'text-slate-500'}`}>{t.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                  ...
                  <TextInput
                    ...
                    className="bg-slate-50 rounded-2xl px-6 py-4 mb-4 text-zinc-900 font-black  uppercase text-xs border border-slate-100"
                  />
                  <TextInput
                    value={annForm.body}
                    ...
                    numberOfLines={3}
                    placeholderTextColor="#94a3b8"
                    className="bg-slate-50 rounded-[32px] px-6 py-4 mb-6 text-zinc-900 font-black  uppercase text-xs border border-slate-100 h-24"
                  />
                  ...
                  <TouchableOpacity onPress={postAnnouncement} disabled={postingAnn} className="rounded-3xl overflow-hidden shadow-lg shadow-pink-500/20">
                    <LinearGradient colors={['#ec4899', '#f43f5e']} className="py-5 items-center justify-center flex-row">
                      {postingAnn ? <ActivityIndicator size="small" color="white" /> : (
                        <>
                          <Text className="text-white font-black  uppercase text-xs tracking-widest mr-3">Send Broadcast</Text>
              ...
              <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm min-h-[300px]">
                <View className="flex-row items-center justify-between mb-8">
                  <Text className="text-zinc-900 font-black  text-xl uppercase tracking-tighter">📡 Live Protocol</Text>
                  ...
                  announcements.slice(0, 10).map((ann, i) => {
                    ...
                    return (
                      <View key={i} className={`mb-6 p-6 rounded-3xl border ${ann.isPinned ? 'bg-indigo-50/20 border-indigo-100' : 'bg-slate-50/50 border-slate-100'}`}>
                        ...
                        <Text className="text-zinc-900 font-black  text-sm uppercase mb-1 leading-5">{ann.Title}</Text>
              ...
              <View className="flex-row gap-4 mb-6">
                ...
                >
                  <LinearGradient colors={['#6366f1', '#4338ca']} className="py-6 items-center">
                    ...
                    <Text className="text-white font-black  text-xs uppercase tracking-widest mt-2">Initialise Team</Text>
                  </LinearGradient>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  ...
                  <Text className="text-indigo-600 font-black  text-xs uppercase tracking-widest mt-2">Search Registry</Text>
                </TouchableOpacity>
              </View>
                  ...
                  return (
                    <View key={i} className="bg-white rounded-[32px] p-6 mb-4 border border-slate-100 flex-row items-center shadow-sm">
                      ...
                      <View className="flex-1">
                        ...
                        <Text className="text-zinc-900 font-black  text-2xl uppercase tracking-tighter mt-1" style={{ color: tint }}>{prize.amount}</Text>
                      </View>
                    </View>
                  );
                })
        ...
        {!isParticipant ? (
          ...
          <View className="flex-row gap-4">
            ...
            >
              ...
              <Text className="text-white font-black  text-[11px] uppercase tracking-widest ml-2">My SQUAD</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text className="text-white font-black  text-[11px] uppercase tracking-widest ml-2">DEPLOY PROJECT</Text>
            </TouchableOpacity>
