...
const FAQItem = ({ faq }: { faq: { question: string; answer: string } }) => {
  ...
  return (
    <TouchableOpacity 
      ...
      className="bg-slate-50 rounded-2xl p-5 mb-3 border border-gray-100"
    >
      <View className="flex-row justify-between items-center">
        <Text className="text-zinc-900 font-black italic uppercase text-xs tracking-tight flex-1">{faq.question}</Text>
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <SafeAreaView className="absolute top-0 left-0 right-0 z-50 pointer-events-none" pointerEvents="box-none">
        <View className="flex-row items-center justify-between px-6 pt-3">
          <TouchableOpacity
            ...
            className="w-12 h-12 rounded-[20px] bg-black/40 items-center justify-center backdrop-blur-md border border-white/20"
          >
          ...
          <View className="flex-row gap-3">
            {isParticipant && (
                ...
                >
                <Text className="text-zinc-900 font-black italic text-[10px] uppercase tracking-widest">My Team</Text>
                </TouchableOpacity>
            )}
            <TouchableOpacity
                onPress={() => navigation.navigate('HackathonLeaderboard', { hackathonId: hackathon._id, hackathonTitle: hackathon.title })}
                className="bg-zinc-900/90 rounded-2xl px-5 py-2.5 shadow-sm border border-zinc-700"
            >
                <Text className="text-white font-black italic text-[10px] uppercase tracking-widest">🏆 Leaderboard</Text>
            </TouchableOpacity>
          </View>
        </View>
      </SafeAreaView>
      ...
      >
          ...
          <View className="absolute bottom-16 px-8">
            <View className="flex-row items-center mb-3">
               <LinearGradient colors={statusMeta.colors as any} className="px-4 py-1.5 rounded-xl border border-white/20">
                  <Text className="text-white font-black italic text-[10px] uppercase tracking-widest">{statusMeta.label}</Text>
               </LinearGradient>
            </View>
            <Text className="text-white text-4xl font-black italic tracking-tighter uppercase leading-[40px] drop-shadow-xl">
              {hackathon.title}
            </Text>
          </View>
        </View>
        ...
        {/* Action Bar */}
        <View className="bg-white -mt-10 mx-6 rounded-[32px] p-6 shadow-2xl shadow-black/10 border border-gray-50 flex-row items-center justify-between">
            <View className="flex-row items-center">
                ...
                <View className="ml-4">
                    <Text className="text-[9px] text-slate-400 font-black uppercase tracking-widest">Organiser</Text>
                    <Text className="text-zinc-900 text-sm font-black italic uppercase">{hackathon.organiser?.name}</Text>
                </View>
            </View>
            <View className="bg-zinc-900 px-5 py-3 rounded-2xl border border-zinc-800">
                <Text className="text-slate-400 text-[8px] font-black uppercase tracking-widest text-center">Prize Pool</Text>
                <Text className="text-white font-black italic uppercase text-sm">{hackathon.prizepool || 'TBA'}</Text>
            </View>
        </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 15 }}>
            {TABS.map((tab, i) => (
              <TouchableOpacity
                ...
                className={`px-6 py-3.5 rounded-2xl border ${activeTab === i ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-100'}`}
              >
                <Text className={`font-black italic text-[10px] uppercase tracking-widest ${activeTab === i ? 'text-white' : 'text-slate-400'}`}>{tab}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View className="mx-6 mb-40">
           ...
           {activeTab === 0 && (
             <View>
                {/* Description */}
                <View className="bg-white rounded-[32px] p-8 mb-6 border border-slate-50 shadow-sm">
                   <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-4">📖 Mission Protocol</Text>
                   ...
                   {hackathon.rules && hackathon.rules.length > 0 && (
                     <View>
                       <Text className="text-zinc-900 font-black italic text-base uppercase tracking-widest mb-4">Commandments</Text>
                       {hackathon.rules.map((rule: string, i: number) => (
                         <View key={i} className="flex-row items-start mb-3">
                           <View className="w-5 h-5 bg-pink-500 rounded-lg items-center justify-center mr-3 mt-0.5"><Text className="text-white text-[10px] font-black">{i+1}</Text></View>
                           <Text className="text-slate-600 text-[13px] leading-5 flex-1">{rule}</Text>
                         </View>
                       ))}
                     </View>
                   )}
                ...
                <View className="flex-row flex-wrap gap-4 mb-6">
                   <View className="flex-1 min-w-[45%] bg-indigo-50/50 rounded-3xl p-6 border border-indigo-100">
                      ...
                      <Text className="text-indigo-400 font-black uppercase text-[9px] tracking-widest mt-4">College Protocols</Text>
                      <Text className="text-indigo-900 font-black italic text-sm uppercase mt-1">
                   ...
                   <View className="flex-1 min-w-[45%] bg-amber-50/50 rounded-3xl p-6 border border-amber-100">
                      ...
                      <Text className="text-amber-400 font-black uppercase text-[9px] tracking-widest mt-4">Registry Size</Text>
                      <Text className="text-amber-900 font-black italic text-sm uppercase mt-1">Up to {hackathon.MaxTeamSize ?? 4} Agents</Text>
                   </View>
                </View>

                {/* Official Seals (Judges & Mentors) */}
                {(hackathon.judges?.length > 0 || hackathon.mentors?.length > 0) && (
                  <View className="bg-zinc-900 rounded-[40px] p-8 mb-6 border border-zinc-800 shadow-xl">
                     <Text className="text-white font-black italic text-base uppercase tracking-widest mb-6">Verified Officials</Text>
                     <View className="flex-row flex-wrap gap-4">
                        {hackathon.judges?.map((j: any, idx: number) => (
                           <View key={`j-${idx}`} className="flex-row items-center bg-zinc-800 rounded-2xl p-2 pr-4 border border-zinc-700">
                              {j.avatar ? <Image source={{uri: j.avatar}} className="w-8 h-8 rounded-xl" /> : <View className="w-8 h-8 rounded-xl bg-zinc-700 items-center justify-center"><Ionicons name="shield" size={12} color="#ec4899" /></View>}
                              <View className="ml-3">
                                 <Text className="text-pink-500 font-black text-[8px] uppercase">Judge</Text>
                                 <Text className="text-white font-bold text-[11px]">{j.name}</Text>
                              </View>
                           </View>
                        ))}
                        {hackathon.mentors?.map((m: any, idx: number) => (
                           <View key={`m-${idx}`} className="flex-row items-center bg-zinc-800 rounded-2xl p-2 pr-4 border border-zinc-700">
                              {m.avatar ? <Image source={{uri: m.avatar}} className="w-8 h-8 rounded-xl" /> : <View className="w-8 h-8 rounded-xl bg-zinc-700 items-center justify-center"><Ionicons name="school" size={12} color="#6366f1" /></View>}
                              <View className="ml-3">
                                 <Text className="text-indigo-500 font-black text-[8px] uppercase">Mentor</Text>
                                 <Text className="text-white font-bold text-[11px]">{m.name}</Text>
                              </View>
                           </View>
                        ))}
                     </View>
                ...
                {hackathon.sponsors?.length > 0 && (
                  <View className="bg-white rounded-[40px] p-8 mb-6 border border-slate-50 shadow-sm">
                     <Text className="text-zinc-900 font-black italic text-base uppercase tracking-widest mb-6">Strategic Partners</Text>
                     <View className="flex-row flex-wrap gap-6">
                        {hackathon.sponsors.map((s: any, idx: number) => (
                           <View key={idx} className="items-center">
                              ...
                              <Text className="text-[9px] font-black uppercase text-slate-400 mt-2 tracking-widest">{s.level}</Text>
                              <Text className="text-[10px] font-bold text-zinc-900 text-center mt-0.5">{s.name}</Text>
                           </View>
                        ))}
                     </View>
                  </View>
                ...
                <View className="bg-slate-50 rounded-[40px] p-8 border border-slate-100 shadow-inner">
                   <Text className="text-zinc-900 font-black italic text-base uppercase tracking-widest mb-6">Official Signals</Text>
                   <View className="gap-3">
                      {hackathon.contactEmail && (
                        <TouchableOpacity className="flex-row items-center bg-white p-4 rounded-2xl shadow-sm">
                           ...
                           <Text className="text-zinc-600 font-black text-[11px] uppercase tracking-widest ml-4">{hackathon.contactEmail}</Text>
                        </TouchableOpacity>
                      )}
                      {hackathon.discordLink && (
                        <TouchableOpacity className="flex-row items-center bg-white p-4 rounded-2xl shadow-sm">
                           <Ionicons name="logo-discord" size={18} color="#5865F2" />
                           <Text className="text-zinc-600 font-black text-[11px] uppercase tracking-widest ml-4">Discord Registry</Text>
                        </TouchableOpacity>
                      )}
                      {hackathon.websiteLink && (
                        <TouchableOpacity className="flex-row items-center bg-white p-4 rounded-2xl shadow-sm">
                           <Ionicons name="globe" size={18} color="#10b981" />
                           <Text className="text-zinc-600 font-black text-[11px] uppercase tracking-widest ml-4">External Portal</Text>
                        </TouchableOpacity>
                      )}
                   </View>
                </View>
           ...
           {/* Tab 1: Timeline */}
           {activeTab === 1 && (
             <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
                <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-8">🕒 Operation Timeline</Text>
                  ...
                  hackathon.timeline.map((event: any, i: number) => (
                    <View key={i} className="flex-row gap-6 mb-8 overflow-visible">
                       <View className="items-center">
                          <View className="w-10 h-10 rounded-2xl bg-zinc-900 items-center justify-center z-10 border-4 border-white shadow-md">
                       ...
                       <View className="flex-1 pt-1 pb-4">
                          <Text className="text-slate-400 font-black uppercase text-[9px] tracking-widest">
                            {new Date(event.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: 'numeric', minute: '2-digit' })}
                          </Text>
                          <Text className="text-zinc-900 font-black italic text-base uppercase tracking-tight mt-1">{event.label}</Text>
                   ...
                   <View className="items-center py-12">
                      ...
                      <Text className="text-slate-400 font-black uppercase text-xs">Standard schedule in execution</Text>
                   </View>
                )}
             </View>
           )}
           ...
           {activeTab === 2 && (
             <View>
                <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-6 mx-2">📡 Strategic Tracks</Text>
                {hackathon.tracks && hackathon.tracks.length > 0 ? (
                  hackathon.tracks.map((track: any, i: number) => (
                    <LinearGradient 
                      key={i}
                      colors={['#fff', '#f8fafc']}
                      className="rounded-[32px] p-8 mb-4 border border-slate-100 shadow-sm"
                    >
                       <View className="flex-row items-center justify-between mb-4">
                         <View className="bg-zinc-900 px-4 py-2 rounded-xl"><Text className="text-white font-black italic uppercase text-[10px]">Track {i+1}</Text></View>
                         <View className="flex-row items-center bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100">
                           ...
                           <Text className="text-emerald-600 font-black italic uppercase text-[10px] ml-2">{track.prizes || 'Standard Bounty'}</Text>
                         </View>
                       </View>
                       <Text className="text-zinc-900 font-black italic text-2xl uppercase tracking-tighter mb-3">{track.title}</Text>
                       <Text className="text-slate-500 text-sm leading-6">{track.description}</Text>
                    </LinearGradient>
                  ))
                ) : (
                  <View className="items-center py-20 bg-white rounded-[40px] border border-slate-50">
                    <Ionicons name="layers-outline" size={48} color="#CBD5E1" />
                    <Text className="text-slate-400 font-black uppercase text-xs mt-4">Open Innovation Track</Text>
                  </View>
                )}
             </View>
           )}
           ...
           {activeTab === 3 && (
             <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
                <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-8">💡 Mission Advisors</Text>
                  ...
                  <View className="flex-row flex-wrap gap-6">
                    {hackathon.mentors.map((mentor: any, i: number) => (
                      <View key={i} className="items-center w-[28%]">
                         <View className="w-16 h-16 rounded-[24px] overflow-hidden border-2 border-slate-100 shadow-sm mb-3">
                         ...
                         <Text className="text-zinc-900 font-black italic text-center uppercase text-[10px] tracking-tight">{mentor.name}</Text>
                         <Text className="text-slate-400 text-[8px] font-black uppercase text-center mt-1">{mentor.role || 'Expert'}</Text>
                      </View>
                    ))}
                  </View>
                ) : (
                  <View className="items-center py-12">
                     <Text className="text-slate-400 font-black uppercase text-xs">Official mentors pending signal</Text>
                  </View>
                )}
             </View>
           )}
           ...
           {activeTab === 4 && (
             <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm">
                <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-8">💬 Protocol FAQ</Text>
                {hackathon.faqs && hackathon.faqs.length > 0 ? (
                  hackathon.faqs.map((faq: any, i: number) => <FAQItem key={i} faq={faq} />)
                ) : (
                  <View className="items-center py-12">
                     <Text className="text-slate-400 font-black uppercase text-xs">Knowledge base expanding...</Text>
                  </View>
                )}
             </View>
           )}
           ...
           {activeTab === 5 && (
             <View>
                {isOfficial() && (
                  <View className="bg-white rounded-[40px] p-8 mb-6 border border-pink-100 shadow-xl shadow-pink-500/5">
                    <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-6">📢 Broadcast Signal</Text>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-6">
                       {ANN_TYPES.map(t => (
                         <TouchableOpacity 
                          ...
                          className={`flex-row items-center px-5 py-3 rounded-2xl border mr-3 ${annForm.type === t.value ? 'bg-zinc-900 border-zinc-800' : 'bg-slate-50 border-slate-100'}`}
                         >
                            <Ionicons name={t.icon as any} size={14} color={annForm.type === t.value ? 'white' : t.color} />
                            <Text className={`font-black italic uppercase text-[10px] tracking-widest ml-2 ${annForm.type === t.value ? 'text-white' : 'text-slate-500'}`}>{t.label}</Text>
                         </TouchableOpacity>
                       ))}
                    </ScrollView>
                    ...
                    <TextInput 
                      ...
                      className="bg-slate-50 rounded-2xl px-6 py-4 mb-4 text-zinc-900 font-black italic uppercase text-xs border border-slate-100"
                    />
                    <TextInput 
                      value={annForm.body} 
                      ...
                      numberOfLines={3}
                      placeholderTextColor="#94a3b8"
                      className="bg-slate-50 rounded-[32px] px-6 py-4 mb-6 text-zinc-900 font-black italic uppercase text-xs border border-slate-100 h-24"
                    />

                    <View className="flex-row items-center justify-between mb-8 px-2">
                       <View className="flex-row items-center">
                          <Ionicons name="pin" size={16} color={annForm.isPinned ? '#ec4899' : '#94a3b8'} />
                          <Text className="text-slate-400 font-black uppercase text-[10px] tracking-widest ml-2">Pin to Priority</Text>
                       </View>
                       <TouchableOpacity 
                    ...
                    <TouchableOpacity onPress={postAnnouncement} disabled={postingAnn} className="rounded-3xl overflow-hidden shadow-lg shadow-pink-500/20">
                       <LinearGradient colors={['#ec4899', '#f43f5e']} className="py-5 items-center justify-center flex-row">
                          {postingAnn ? <ActivityIndicator size="small" color="white" /> : (
                            <>
                              <Text className="text-white font-black italic uppercase text-xs tracking-widest mr-3">Send Broadcast</Text>
                ...
                {/* Announcement Feed */}
                <View className="bg-white rounded-[40px] p-8 border border-slate-50 shadow-sm min-h-[300px]">
                   <View className="flex-row items-center justify-between mb-8">
                      <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter">📡 Live Protocol</Text>
                      <TouchableOpacity onPress={() => navigation.navigate('HackathonChannel', { hackathonId, hackathonTitle: hackathon.title })}>
                         <Text className="text-pink-500 font-black uppercase text-[10px] tracking-widest">Full History →</Text>
                      </TouchableOpacity>
                   </View>

                   {annLoading ? <ActivityIndicator size="large" color="#ec4899" className="py-12" /> : announcements.length > 0 ? (
                          ...
                          <View key={i} className={`mb-6 p-6 rounded-3xl border ${ann.isPinned ? 'bg-indigo-50/20 border-indigo-100' : 'bg-slate-50/50 border-slate-100'}`}>
                             <View className="flex-row items-center justify-between mb-3">
                                <View className={`px-3 py-1 rounded-xl border flex-row items-center`} style={{ backgroundColor: type.bg, borderColor: type.color + '20' }}>
                                   ...
                                   <Text className="text-[9px] font-black uppercase ml-1.5" style={{ color: type.color }}>{type.label}</Text>
                                </View>
                                {ann.isPinned && <Ionicons name="pin" size={14} color="#6366f1" />}
                             </View>
                             <Text className="text-zinc-900 font-black italic text-sm uppercase mb-1 leading-5">{ann.Title}</Text>
                             ...
                             <View className="flex-row items-center">
                                ...
                                <Text className="text-slate-400 text-[9px] font-black uppercase ml-2">{ann.author?.name} • {new Date(ann.createdAt).toLocaleDateString()}</Text>
                             </View>
                          </View>
                        );
                      })
                      ...
                      <View className="items-center py-20">
                         ...
                         <Text className="text-slate-400 font-black uppercase text-xs mt-4">No signals received</Text>
                      </View>
                   )}
                </View>
             </View>
             ...
             <View>
                <View className="flex-row gap-4 mb-6">
                  <TouchableOpacity
                    ...
                    className="flex-1 rounded-[28px] overflow-hidden shadow-lg shadow-indigo-500/20"
                  >
                    <LinearGradient colors={['#6366f1', '#4338ca']} className="py-6 items-center">
                      <Ionicons name="add-circle" size={24} color="white" />
                      <Text className="text-white font-black italic text-xs uppercase tracking-widest mt-2">Initialise Team</Text>
                    </LinearGradient>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() => navigation.navigate('HackathonTeamScreen', { hackathonId: hackathon._id, mode: 'browse' })}
                    className="flex-1 rounded-[28px] bg-white border border-slate-100 py-6 items-center shadow-sm"
                  >
                    <Ionicons name="search" size={24} color="#6366f1" />
                    <Text className="text-indigo-600 font-black italic text-xs uppercase tracking-widest mt-2">Search Registry</Text>
                  </TouchableOpacity>
                </View>
                <View className="bg-indigo-900 rounded-[32px] p-8 border border-indigo-800 shadow-xl shadow-indigo-500/10">
                   <View className="flex-row items-center mb-4">
                      <Ionicons name="flash" size={20} color="#818cf8" />
                      <Text className="text-white font-black italic text-base uppercase ml-2">Fync Engine v.2.0</Text>
                   </View>
           ...
           {activeTab === 7 && (
             <View>
                <Text className="text-zinc-900 font-black italic text-xl uppercase tracking-tighter mb-6 mx-2">🏆 Bounty Distribution</Text>
                {hackathon.prizes && hackathon.prizes.length > 0 ? (
                  hackathon.prizes.map((prize: any, i: number) => {
                    ...
                    const medals = ['🥇', '🥈', '🥉'];
                    return (
                      <View key={i} className="bg-white rounded-[32px] p-6 mb-4 border border-slate-100 flex-row items-center shadow-sm">
                        <View className="w-20 h-20 rounded-[28px] items-center justify-center mr-6 border border-slate-50" style={{ backgroundColor: bg }}>
                          <Text className="text-4xl">{medals[i] ?? `#${prize.rank}`}</Text>
                        </View>
                        <View className="flex-1">
                          <Text className="text-slate-400 font-black uppercase text-[9px] tracking-widest">{prize.title}</Text>
                          <Text className="text-zinc-900 font-black italic text-2xl uppercase tracking-tighter mt-1" style={{ color: tint }}>{prize.amount}</Text>
                        </View>
                      </View>
                    );
                  })
                  ...
                  <View className="items-center py-20 bg-white rounded-[40px] border border-slate-50">
                    ...
                    <Text className="text-slate-400 font-black uppercase text-xs mt-4">Prizes in allocation phase</Text>
                  </View>
                )}
             </View>
           )}
      ...
      <View className="absolute bottom-0 left-0 right-0 px-6 pb-10 pt-4 bg-white/95 border-t border-slate-50 backdrop-blur-xl">
          {!isParticipant ? (
                ...
                <View className="py-5 flex-row items-center justify-center">
                  {joining ? (
                    ...
                    <>
                      ...
                      <Text className="text-white font-black italic text-base uppercase tracking-widest ml-3">Join Ecosystem Registry</Text>
                    </>
                  )}
                </View>
              </LinearGradient>
            ...
            <View className="flex-row gap-4">
              <TouchableOpacity
                onPress={() => navigation.navigate('HackathonTeamScreen', { hackathonId: hackathon._id })}
                className="flex-[1.5] rounded-24px bg-zinc-900 py-5 items-center justify-center shadow-lg shadow-black/10 flex-row"
                style={{ borderRadius: 24 }}
              >
                <Ionicons name="people" size={18} color="white" />
                <Text className="text-white font-black italic text-[11px] uppercase tracking-widest ml-2">My SQUAD</Text>
              </TouchableOpacity>
              ...
              >
                ...
                <Text className="text-white font-black italic text-[11px] uppercase tracking-widest ml-2">DEPLOY PROJECT</Text>
              </TouchableOpacity>
              ...
              {isOfficial() && (
                <>
                  <TouchableOpacity
                    ...
                    className="w-16 rounded-24px bg-zinc-900 items-center justify-center shadow-lg border border-zinc-800"
                    style={{ borderRadius: 24 }}
                  >
