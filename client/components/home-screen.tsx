...
export default function HomeScreen() {
    ...
    <View className="w-full flex-row border-b border-gray-700">
      {['For You', 'Following'].map((tabTitle) => {
        ...
        return (
          ...
          >
            <Text className={`text-base font-semibold ${isActive ? 'text-white' : 'text-gray-400'}`}>
              {tabTitle}
            </Text>
            {isActive && (
              <View
  ...
  const renderPostItem = ({ item }) => (
    <View className="mb-5 bg-white/10 rounded-xl">
      <View className="flex-row items-center justify-between px-4 py-3">
        <View className="flex-row items-center">
          <Image
        ...
        <View className='flex-row items-center gap-3'>
          <TouchableOpacity className='border border-pink-300 px-3 py-1 rounded-lg'>
      ...
      <View className='px-3 py-2'>
        <Text className="text-pink-300 text-md">{item.description}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-black">
      ...
      <TouchableOpacity
        className="absolute bottom-32 right-8 h-14 w-14 items-center justify-center rounded-full bg-pink-300 shadow-lg"
