    ...
    const renderMessage = ({ item }: { item: any }) => {
        ...
        return (
            <View className={`mb-4 flex-row ${isMine ? 'justify-end' : 'justify-start'}`}>
                {!isMine && (
                    <Image source={{ uri: item.sender?.avatar || 'https://via.placeholder.com/150' }} className="w-8 h-8 rounded-full bg-zinc-100 mr-2" />
                )}
                <View className={`max-w-[80%] rounded-2xl px-4 py-3 ${isMine ? 'bg-zinc-200' : 'bg-white border border-zinc-100'}`}>
                    {!isMine && (
                        <Text className="text-black font-bold text-[10px] mb-1">{item.sender?.name} {isAdmin && '• [Admin]'}</Text>
                    )}
                    ...
                    {item.fileUrl && (
                        <View className="mb-2">
                            {item.fileType === 'image' ? (
                                ...
                                <View className="bg-zinc-50 p-2 rounded-lg flex-row items-center border border-zinc-100">
                                    <Feather name="file" size={20} color="#71717a" />
                                    <Text className="ml-2 text-zinc-600 text-xs font-bold" numberOfLines={1}>{item.fileName}</Text>
                                </View>
                            )}
                        </View>
                    )}
                    ...
                    {item.isPoll ? (
                        <View className="w-56">
                            <Text className="text-zinc-900 font-bold text-sm mb-3">{item.pollQuestion}</Text>
                            {item.pollOptions.map((opt: any, idx: number) => {
                                ...
                                return (
                                    <TouchableOpacity 
                                        ...
                                        className={`mb-2 bg-zinc-50 rounded-lg overflow-hidden border ${hasVoted ? 'border-blue-400 bg-blue-50/30' : 'border-zinc-100'}`}
                                    >
                                        <View className="flex-row items-center justify-between p-3 relative">
                                            ...
                                            <View className="flex-row items-center z-10 flex-1">
                                                <Text className={`text-xs font-bold ${hasVoted ? 'text-blue-900' : 'text-zinc-800'}`}>{opt.optionText}</Text>
                                                {hasVoted && (
                                                    <Ionicons name="checkmark-circle" size={14} color="#1d4ed8" style={{ marginLeft: 6 }} />
                                                )}
                                            </View>
                                            <Text className={`text-[10px] font-black z-10 ${percentage === 100 ? 'text-white' : 'text-zinc-500'}`}>{Math.round(percentage)}%</Text>
                                        </View>
                                    </TouchableOpacity>
                                );
                            })}
                            <View className="mt-2 pt-2 border-t border-zinc-100 flex-row items-center justify-between">
                                <View className="bg-zinc-100 px-3 py-1 rounded-full border border-zinc-200">
                                    <Text className="text-zinc-500 text-[7px] font-black uppercase tracking-tight">
                        ...
                        <Text className={`text-md ${isMine ? 'text-black' : 'text-zinc-800'}`}>{item.text}</Text>
                    )}

                    <View className="flex-row justify-end mt-1">
                         <Text className={`text-[8px] font-bold ${isMine ? 'text-zinc-400' : 'text-zinc-300'}`}>
    ...
    return (
        <View className="flex-1 bg-white">
            ...
            <SafeAreaView className="flex-1" edges={['top', 'bottom']}>
                {/* Header */}
                <View className="px-5 py-4 flex-row items-center border-b border-zinc-50">
                    ...
                    <View className="ml-4 flex-1">
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-zinc-900 font-black uppercase text-xs">{subGroupName}</Text>
                        </View>
                        <Text className="text-green-500 font-bold text-[8px] uppercase tracking-widest">{subGroup?.isGeneral ? 'Community Hub' : 'Connected Now'}</Text>
                    </View>
                        ...
                        {canType ? (
                            <View className="flex-row items-end gap-3">
                                <TouchableOpacity 
                                    ...
                                    className="w-11 h-11 bg-zinc-100 rounded-2xl items-center justify-center mb-1"
                                >
                                    <Feather name="paperclip" size={20} color="#71717a" />
                                </TouchableOpacity>
                                <View className="flex-1 min-h-[40px] max-h-32 bg-zinc-100 rounded-2xl px-4 border border-zinc-100 justify-center">
                                    <TextInput 
                                ...
                                <TouchableOpacity 
                                    ...
                                    className={`w-11 h-11 rounded-2xl items-center justify-center mb-1 ${text.trim() ? 'bg-black' : 'bg-zinc-200'}`}
                                >
                                    {sending ? <ActivityIndicator color="white" size="small" /> : <Feather name="arrow-up" size={24} color="white" />}
                                </TouchableOpacity>
                            </View>
                        ) : (
                            <View className="bg-zinc-50 py-4 rounded-2xl items-center justify-center border border-dashed border-zinc-200">
                                <Feather name="lock" size={14} color="#a1a1aa" />
                                <Text className="text-zinc-400 text-[10px] font-black uppercase tracking-widest mt-1">Only admins can send messages</Text>
                            </View>
                        )}
                    </View>
                    ...
                    <View className="flex-1 bg-black/60 justify-end">
                        <View className="bg-white rounded-t-[40px] p-8">
                            <View className="flex-row justify-between items-center mb-6">
                                <Text className="text-zinc-900 text-xl font-black uppercase tracking-widest">Create Poll</Text>
                            ...
                            <TextInput 
                                ...
                                className="bg-zinc-100 p-4 rounded-2xl mb-4 font-bold"
                                value={pollQuestion}
                            ...
                            {pollOptions.map((opt, idx) => (
                                <TextInput 
                                    ...
                                    className="bg-zinc-100 p-4 rounded-xl mb-3"
                                    value={opt}
                                    onChangeText={(val) => {
