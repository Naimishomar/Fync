...
export default function Shorts() {
  ...
  const videoRefs = useRef(new Map<string, Video>());
  ...
  const togglePlayForId = async (id: string) => {
    try {
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: ViewToken[] }) => {
    ...
    viewableIndex.current = visible.index || 0;
  }).current;
  ...
  const renderItem = ({ item }: { item: ShortItem }) => {
    return (
      <View className="w-full h-screen bg-black">
        ...
        <View className={`absolute left-4 right-4 flex-row justify-between items-end ${isTablet ? "bottom-36" : "bottom-14"}`}>
          ...
          <View className="w-16 items-center justify-end">
            <TouchableOpacity className="mb-7">
              <Ionicons name={like ? "heart" : "heart-outline"} size={30} color={like ? "red" : "#fff"} onPress={()=> setLike(prev=> !prev)}/>
            </TouchableOpacity>
            <TouchableOpacity className="mb-7">
